...
matrix-cli accountable register \
  ...
  --requirements SW-REQ-001 \
  --tools terminal,file \
  --files conduvera/skills/accountable_agent/__init__.py
...
ac = service.register_accountable_change(
    ...
    change_intent=ChangeIntent(
        description="Implement accountable agent layer",
        change_type="feature",
        files_affected=["conduvera/skills/accountable_agent/__init__.py"],
    ),
    cr_id="CR-001",
    requirement_refs=["SW-REQ-001"],
)
...
```bash
python3 -m pytest conduvera/skills/accountable_agent/tests -q
```
