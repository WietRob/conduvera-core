...
Usage:
    conduvera safety check --delete file.txt
    conduvera cr create --title "Fix bug"
    conduvera session start --project ~/myproject
    conduvera aspice check
    conduvera lock claim --file src/main.py --agent cursor
"""
...
def cr_create(
    ...
    try:
        from conduvera.skills.change_request import ChangeRequestService
...
def cr_evidence(
    ...
    try:
        from conduvera.skills.change_request import generate_cr_evidence
...
def cr_validate(
    ...
    try:
        from conduvera.skills.change_request import validate_cr_traceability
...
def session_start(
    ...
    try:
        ...
        sm = AgentSessionManager(storage_dir=Path.home() / ".conduvera" / "sessions")
...
def lock_claim(
    ...
    """Claim a file lock."""
    try:
        from conduvera.skills.multi_agent_lock import MultiAgentLock, LockScope
        
        lock_mgr = MultiAgentLock(storage_dir=Path.home() / ".conduvera" / "locks")
...
def pattern_record(
    ...
    try:
        from conduvera.skills.pattern_learning import PatternLearningEngine, CodeFinding
        
        pl = PatternLearningEngine(storage_dir=Path.home() / ".conduvera" / "patterns")
