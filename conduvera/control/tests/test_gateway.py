...
from conduvera.control.gateway.auth import ClientConfig, ClientRegistry
from conduvera.control.gateway.backends import BackendProxy
from conduvera.control.gateway.config import (
    GatewayConfig,
    ProfileConfig,
    SensitiveClass,
)
from conduvera.control.gateway.router import GatewayRouter, PolicyDecision
from conduvera.control.gateway.audit import AuditEntry, AuditLogger
...
def _make_sensitive_classes() -> Dict[str, SensitiveClass]:
    ...
    return {
        "conduvera": SensitiveClass(name="conduvera", cloud_allowed=False),
...
def _make_client_registry() -> ClientRegistry:
    ...
    registry._clients = {
        "test-client": ClientConfig(
            ...
            cloud_allowed=False,
            sensitive_classes=["conduvera", "private_repo"],
        ),
...
class TestClientRegistryYaml:
    ...
    def test_laden_aus_yaml(self, tmp_path: Path) -> None:
        ...
        assert "conduvera" in client.sensitive_classes
...
class TestChatCompletions:
    ...
    def test_chat_sensitive_class_blocked(
        ...
        """Sensitive-Class conduvera darf nicht an Cloud."""
        ...
        resp = app_client.post(
            ...
            json={
                ...
                "sensitive_class": "conduvera",
            },
        )
        # conduvera hat cloud_allowed=False, cloud_public hat provider=openai
        assert resp.status_code == 403
...
class TestPolicyDurchsetzung:
    ...
    def test_sensitive_class_blockiert_cloud(
        ...
        """Sensitive-Class conduvera muss Cloud blockieren."""
        router = GatewayRouter(config)
        decision = router.route("cloud_public", sensitive_class="conduvera")
    ...
    def test_sensitive_class_erlaubt_lokal(
        ...
        router = GatewayRouter(config)
        decision = router.route("local_deep", sensitive_class="conduvera")
        assert decision.policy_decision == PolicyDecision.ALLOWED
...
class TestConfigValidation:
    ...
    def test_is_cloud_allowed_for_class(
        ...
        assert config.is_cloud_allowed_for_class("conduvera") is False
