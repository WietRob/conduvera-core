...
| Route-plan candidate ranking | Code editor / split pane / route-plan panel | displays existing `route-plan.v1` JSON artifacts through a read-only viewer stub, exact panel snapshot fixtures, a canonical artifact selector, and a discoverable non-live Matrix UI route-plan panel |
