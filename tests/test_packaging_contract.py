...
def _setuptools_package_includes() -> list[str]:
    ...
    for raw_line in _pyproject_text().splitlines():
        ...
        if in_find and line.startswith("include"):
            ...
            includes.extend(part.strip().strip('"') for part in line.split("[", 1)[1].split("]", 1)[0].split(",") if part.strip())
            if "]" in line:
                collecting_include = False
            continue
        if collecting_include:
            includes.extend(part.strip().strip('"') for part in line.split("]", 1)[0].split(",") if part.strip())
            if "]" in line:
                collecting_include = False
    return includes
