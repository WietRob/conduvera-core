...
from curaops.control.launcher import AgentLauncher
from curaops.control.scripts_bridge import (
    ScriptRunner,
    ScriptConfig,
...
class TestBlockedStreamEnforcement:
    ...
    @patch("curaops.control.scripts_bridge.subprocess.run")
...
class TestLauncherAlreadyActive:
    ...
    def test_aktiver_launch_verweigert(self, launcher: AgentLauncher) -> None:
        launcher._registry.update("Batman", status=AgentStatus.ACTIVE, session="tmux:curaops-Batman")
        result = launcher.launch("Batman")
        assert result.success is False
