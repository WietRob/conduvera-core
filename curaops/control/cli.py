...
from curaops.control.eventlog import EventLog
from curaops.control.gates import GateRunner, GateStatus, BUILTIN_GATES
from curaops.control.stream_state import StreamStateStore, StreamState, AgentReply
...
def agent_ready(
    ...
    from curaops.control.scripts_bridge import ScriptRunner, parse_readiness_stdout
    from curaops.control.worktree_sentinel import WorktreeSentinel
    ...
    if not legacy_only:
        from curaops.control.gates import GateRunner
...
def gate_run(
    ...
    if use_legacy:
        ...
        from curaops.control.scripts_bridge import ScriptRunner, ScriptName, ExitCode
        from curaops.control.worktree_sentinel import WorktreeSentinel
...
def gate_profiles():
    ...
    if not profiles:
        ...
        console.print("Create: .curaops/control/policies/gates.yaml")
        return
...
def _get_adapter(tool: str):
    ...
    if tool == "opencode":
        from curaops.control.adapters.opencode import OpenCodeAdapter
...
def gateway_route(
    ...
    from curaops.control.gateway.config import load_gateway_config
    from curaops.control.gateway.router import GatewayRouter
    from curaops.control.gateway.audit import AuditLogger
...
def gateway_smoke():
    ...
    from curaops.control.gateway.config import load_gateway_config
    from curaops.control.gateway.smoke import smoke_check_all
...
def gateway_serve(
    ...
    from curaops.control.gateway.config import load_gateway_config
    from curaops.control.gateway.auth import ClientRegistry
    from curaops.control.gateway.audit import AuditLogger
    from pathlib import Path
