"""Internal Buildroom no-progress reconciliation guard (Conduvera Core).

Ported 1:1 from the frozen legacy component
`legacy/buildroom/source/buildroom_no_progress.py` (sha256
e11034326f7f47f5e70e0b5acff84ab7a06f8094689f57a8726b7b7fab7ac7a7,
98 lines) — behaviour/state parity is proven by differential tests in
`tests/buildroom/test_no_progress_differential.py`.

SCOPE (hard boundary):
- no_progress is an internal Buildroom guard of Conduvera Core.
- It owns NO task/session/harness/model/runtime/secrets authority.
...
- `NoProgressResult`   frozen dataclass (count, terminal_hold, fingerprint)
- `observe_reconciliation(state, *, phase, status, blocker, task_id,
  task_board="", evidence_fingerprint, log_fingerprint, threshold=3)`
